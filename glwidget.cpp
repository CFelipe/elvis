...
void rotacionaObjeto(Objeto *ob){
    ...
    if (ob->selecionado){
        ...
        case Objeto::POLILINHA:
        {
            ...
            while (linha!=NULL){
                Ponto P0 = linha->getP0().p;
                Ponto P1 = linha->getP1().p;
...
void deslocaPontoPolilinha(Polilinha *pol, int x, int y){
    ...
    if (sel1->getP0().selecionado){ // linha 1, portanto sel2=NULL
        sel1->setP0(Ponto(sel1->getP0().p.x+dx, sel1->getP0().p.y+dy));
   } else if (sel1->getP1().selecionado){
        sel1->setP1(Ponto(sel1->getP1().p.x+dx, sel1->getP1().p.y+dy));
        if (sel2!=NULL){
           sel2->setP0(Ponto(sel2->getP0().p.x+dx, sel2->getP0().p.y+dy));
        }
    }
    pol->atualizaMINMAX();
   pol->setXClick(x);
...
void excluirPontoControlePolilinha(Polilinha *pol, int x, int y){
    if (clippingAreaVertice(pol->getInit()->getP0().p, Ponto(x, y))){
        pol->remove(pol->getInit());
    } else {
        Linha *linha = pol->getInit();
        while (linha!=NULL){
            if (clippingAreaVertice(linha->getP1().p, Ponto(x, y))){
    ...
    if (pol->getInit()!=NULL) pol->atualizaMINMAX();
            ...
            if (aux->selecionado){
                if (aux->getTipo() == Objeto::CIRCULO){
                    ...
                    if (op==ESCALA){
                        ...
                        glLineStipple(1, 0xAAAA);
                        glBegin(GL_LINES);
                            glVertex2i(pol->getMax().x+CONTROL, pol->getMax().y+CONTROL);
                            glVertex2i(pol->getMax().x+CONTROL, pol->getMin().y-CONTROL);
                            glVertex2i(pol->getMax().x+CONTROL, pol->getMin().y-CONTROL);
                            glVertex2i(pol->getMin().x-CONTROL, pol->getMin().y-CONTROL);
                            glVertex2i(pol->getMin().x-CONTROL, pol->getMin().y-CONTROL);
                            glVertex2i(pol->getMin().x-CONTROL, pol->getMax().y+CONTROL);
                            glVertex2i(pol->getMin().x-CONTROL, pol->getMax().y+CONTROL);
                            glVertex2i(pol->getMax().x+CONTROL, pol->getMax().y+CONTROL);
                        glEnd();
                        glDisable(GL_LINE_STIPPLE);
                        glBegin(GL_POINTS);
                            glColor3i(0,0,0);
                            glVertex2i(pol->getMax().x, pol->getMax().y);
                            glVertex2i(pol->getMax().x, pol->getMin().y);
                            glVertex2i(pol->getMin().x, pol->getMin().y);
                            glVertex2i(pol->getMin().x, pol->getMax().y);
        ...
        if (desenha){
            if (onMouseClick==false){
                if (forma == Objeto::CIRCULO){
                    ...
                    GLfloat color[4] = {1, 0, 0, 0};
                    Polilinha *pol = new Polilinha(color, color, espessuraLinha);
                ...
                if (forma == Objeto::POLILINHA){
                    ...
                    pol->insert(pol->getFim()->getP1().p, click, pol->getFim());
            ...
            if (preAgrupamento==false){
                    ...
                    for(j = 0; j < sizeJ; j++) {
                        ...
                        if (aux->getTipo() == Objeto::CIRCULO){
                            ...
                            while (linha!=NULL && pol->selecionado == false){
                                if (cohenClipping(linha->getP0().p, linha->getP1().p, clik->C.p, clik->B.p)){
                                    pol->selecionado = (true);
                                    aux->setXClick(event->x());
                                    aux->setYClick(gfWrldSizeY-event->y());
                ...
                if (oneAnySelection==false){
                    ...
                    GLfloat colorFill[4] = {1, 0, 0, 0};
                    GLfloat colorLine[4] = {0, 0, 0, 0};
                    Retangulo *q = new Retangulo(A, B, C, D, colorFill, colorLine, 1);
                    q->isSeletor = true;
                    q->isVisible = true;
                    camadaSelecionada->objetos->append(q);
                    preAgrupamento=false;
                    ...
                    if (aux->getTipo() == Objeto::CIRCULO){
                        ...
                        while (linha!=NULL && pol->selecionado == false){
                            ...
                            if (cohenClipping(linha->getP0().p, linha->getP1().p, clik->C.p, clik->B.p)){
                                ...
                                if (clippingAreaVertice(linha->getP0().p, Ponto(event->x(),gfWrldSizeY-event->y()))){
                                    linha->getPP0()->selecionado = (true);
                                    cout<<"Seelcionado: P0 - Linha 1"<<endl;
                                } else {
                                    if (clippingAreaVertice(linha->getP1().p, Ponto(event->x(),gfWrldSizeY-event->y()))){
                                        ...
                                        if (linha!=pol->getFim()){
                                            ...
                                            linha->getNext()->getPP1()->selecionado = false;
                        ...
                        if (pol->selecionado==false && op==ESCALA){
                            ...
                            if (op==COPIA){
                                ...
                                Polilinha *novo = new Polilinha(fill, line, espessuraLinha);
                                camadaSelecionada->objetos->append(novo);
                                Linha *linha = pol->getInit();
                                while (linha!=NULL){
                                    novo->insert(linha->getP0().p, linha->getP1().p, novo->getFim());
                                    linha = linha->getNext();
                                }
                                aux->selecionado = (false);
                                ...
                                Ponto newP1(pol->getLinhaSelecionada1()->getP1().p.x, pol->getLinhaSelecionada1()->getP1().p.y);
                                ...
                                pol->getLinhaSelecionada1()->getPP0()->selecionado = false;
                                ...
                                pol->getLinhaSelecionada2()->getPP1()->selecionado = (false);
...
void GLWidget::mouseReleaseEvent(QMouseEvent *event){
    if (desenha==false){
        onMouseClick = false;
        if (op==SELECIONAR && camadaSelecionada->objetos->last()->getTipo()==Objeto::RETANGULO){
            //camadaSelecionada->objetos->removeLast();
    ...
    if (desenha){
        if(onMouseClick){
            if (camadaSelecionada->objetos->last()->getTipo() == Objeto::CIRCULO){
                ...
                Polilinha *pol = dynamic_cast <Polilinha *>(camadaSelecionada->objetos->last());
                Ponto click(event->x(),gfWrldSizeY-event->y());
                pol->setFim(pol->getFim()->getP0().p, click);
            }
        }
                    ...
                    if (aux->selecionado){
                        if (op==TRANSLACAO || op==COPIA){
                            if (aux->getTipo() == Objeto::CIRCULO){
                                ...
                                while (linha!=NULL){
                                    linha->setP0(Ponto(linha->getP0().p.x+dx, linha->getP0().p.y+dy));
                                    linha->setP1(Ponto(linha->getP1().p.x+dx, linha->getP1().p.y+dy));
                                    linha = linha->getNext();
                                }
                                pol->atualizaMINMAX();
                                pol->setXClick(event->x());
...
void GLWidget::setOperacao(QAction* q) {
    ...
    if(q->text() == "Retângulo") {
        ...
        if (preAgrupamento==true){
            Objeto* aux;
            int i, j;
            Retangulo *q2 = dynamic_cast <Retangulo *>(camadaSelecionada->objetos->last());
            camadaSelecionada->objetos->removeLast();
            for(i = 0; i < camadas.size(); i++) {
                GLint sizeJ = camadas.at(i)->objetos->size();
                for(j = 0; j < sizeJ; j++) {
                    ...
                    if (aux->selecionado){
                        if (aux->getTipo() == Objeto::CIRCULO){
                            ...
                            c->getColorFill(fill);
                            c->getColorLine(line);
                            Circulo *novo = new Circulo(c->getRaio(), c->getXc()+CONTROL, c->getYc()+CONTROL, fill,line, c->getEspessuraLinha());

                            aux->selecionado = (false);

                            camadaSelecionada->objetos->append(novo);
                            camadaSelecionada->objetos->last()->selecionado = true;
                            camadaSelecionada->objetos->last()->setXClick(c->getXClick());
                            camadaSelecionada->objetos->last()->setYClick(c->getYClick());
                            ...
                            e->getColorFill(fill);
                            e->getColorLine(line);
                            Elipse *novo = new Elipse(Ponto(e->getCentro().p.x+CONTROL, e->getCentro().p.y+CONTROL), e->getRaioHorizontal(), e->getRaioVertical(), fill, line, espessuraLinha);
                            novo->setControl(e->getControl());
                            aux->selecionado = (false);
                            camadaSelecionada->objetos->append(novo);
                            camadaSelecionada->objetos->last()->selecionado = (true);
                            ...
                            Retangulo *ret = dynamic_cast <Retangulo *>(aux);
                            GLfloat fill[4], line[4];
                            ret->getColorFill(fill);
                            ret->getColorLine(line);
                            Retangulo *novo = new Retangulo(Vertice(ret->A.p.x+CONTROL, ret->A.p.y+CONTROL), Vertice(ret->B.p.x+CONTROL, ret->B.p.y+CONTROL), Vertice(ret->C.p.x+CONTROL, ret->C.p.y+CONTROL), Vertice(ret->D.p.x+CONTROL, ret->D.p.y+CONTROL), fill, line, espessuraLinha);

                            camadaSelecionada->objetos->append(novo);

                            aux->selecionado = (false);
                            camadaSelecionada->objetos->last()->selecionado = true;
                            camadaSelecionada->objetos->last()->setXClick(ret->getXClick());
                            camadaSelecionada->objetos->last()->setYClick(ret->getYClick());
                        } else if (aux->getTipo() == Objeto::POLILINHA){
                            Polilinha *pol = dynamic_cast <Polilinha *>(aux);
                            ...
                            Linha *linha = pol->getInit();
                            while (linha!=NULL){
                                novo->insert(Ponto(linha->getP0().p.x+CONTROL,linha->getP0().p.y+CONTROL), Ponto(linha->getP1().p.x+CONTROL,linha->getP1().p.y+CONTROL), novo->getFim());
                                linha = linha->getNext();
                            }
                            aux->selecionado = (false);
            ...
            q2->setA(Vertice(q2->A.p.x+CONTROL, q2->A.p.y+CONTROL));
            q2->setB(Vertice(q2->B.p.x+CONTROL, q2->B.p.y+CONTROL));
            q2->setC(Vertice(q2->C.p.x+CONTROL, q2->C.p.y+CONTROL));
            q2->setD(Vertice(q2->D.p.x+CONTROL, q2->D.p.y+CONTROL));
            camadaSelecionada->objetos->append(q2);
            q->setChecked(false);
